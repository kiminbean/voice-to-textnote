...
| Phase 3 (의존성 주입) | ✅ 구현 완료 | 31개 라우트에서 `Depends()` 사용, 22개 factory function (`get_X_service()`) 패턴으로 Depends() 호환. 유일한 모듈레벨 singleton (`devices.py:_push_service`) → `_get_push()` factory로 전환 완료 |
| Phase 4 (라우터 그룹핑) | ⏭️ 연기 | flat 구조 유지 (35개 라우터). Priority Low + 높은 회귀 리스크(158+ 테스트 import 변경)로 post-deployment로 연기 |

### Test Status (2026-06-05 Production Ready)

- **Total**: 2494 passed / 0 failed / 4 skipped
- **Coverage**: 97.67%
- **ruff**: All checks passed

### Known Regressions: 0건 (전부 해결)

CI 테스트 그린화(14건) + torchaudio mock(4건) + retention(1건) = 19건 전부 수정 완료
...
전체 스위트 실행 결과 **27건 실패**가 관측되었다. 당시 **9건은 `tests/e2e/test_pipeline_e2e.py`의 `RuntimeError: There is no current event loop` 오류로, 리팩토링과 무관한 Python 3.14 환경(asyncio 이벤트 루프) 이슈**로 분류되어 Scope에서 제외했다. 이 제외 조건은 2026-06-15 후속 재검증에서 해소되었고 focused e2e는 `16 passed`로 통과한다. 리팩토링 관련 회귀는 다음 3개 근본 원인 카테고리로 분류된다.
...
**Ubiquitous**: 시스템은 **항상** 백엔드 테스트 스위트와 focused pipeline e2e를 통과해야 한다.

- 수용 검증: `venv/bin/python -m pytest backend -q` → **3323 passed, 16 skipped** (2026-06-14), `venv/bin/python -m pytest -o addopts="" backend/tests/e2e/test_pipeline_e2e.py -q` → **16 passed** (2026-06-15)
...
| AC-5 | `venv/bin/python -m pytest -o addopts="" backend/tests/e2e/test_pipeline_e2e.py -q` + `venv/bin/python -m pytest backend -q` | ✅ **충족 (2026-06-15 재검증)** — focused e2e `16 passed`; 전체 backend suite도 2026-06-14 `3323 passed, 16 skipped` |
...
- **e2e 9건 (`test_pipeline_e2e.py`) — 후속 완료**: 과거에는 `RuntimeError: There is no current event loop`로 제외했으나 현재 Python 3.14 venv에서 재현되지 않는다. 2026-06-15 재검증: `venv/bin/python -m pytest -o addopts="" backend/tests/e2e/test_pipeline_e2e.py -q` -> `16 passed, 6 warnings in 1.73s`.
...
- e2e 이벤트 루프 이슈 9건(`tests/e2e/test_pipeline_e2e.py`)은 과거 Python 3.14 asyncio 환경 이슈로 제외했으나, 2026-06-15 재검증에서는 재현되지 않는다. 현재 전체 게이트는 E2E 포함 `pytest tests/ -q`로 실행한다.
...
- **e2e (`test_pipeline_e2e.py`)**: 과거에는 Python 3.14 asyncio 이벤트 루프 이슈로 제외했으나, 2026-06-15 재검증에서 재현되지 않음. `venv/bin/python -m pytest -o addopts="" backend/tests/e2e/test_pipeline_e2e.py -q` -> `16 passed, 6 warnings in 1.73s`; 전체 backend suite도 E2E 포함 `3323 passed, 16 skipped`, coverage `98.62%`.
