...
mypy를 품질 게이트에 추가한 직후 측정한 초기 baseline은 **40개 파일에 걸친 194개 타입 에러**였다.

> **Baseline 갱신 (2026-06-13)**: 코드베이스 성장으로 현재 **219개 에러**로 증가. 테스트는 3621 passed / 커버리지 100%로 런타임 영향 없음. SPEC 상태를 `in_progress`로 갱신.

| 분류 | 초기 (2026-06-08) | 현재 (2026-06-13) |
|------|-------------------|-------------------|
| 전체 | 194 (40 파일) | 219 |
| 프로덕션 코드 | 170 | — |
| 테스트 파일 | 24 | — |
