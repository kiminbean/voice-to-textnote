...
- **정확도 기준선**: 2026-07-02 v14 기준 `promise_radar_accuracy_cases.json` 193건 중 126건은 Creative Commons Town Meeting TV 실제 회의/녹음 파생 label이며 evaluator 결과는 193/193, accuracy 1.0입니다. 출처, subtitle cache, 대표 음성 clip 재생성 명령은 `backend/tests/fixtures/promise_radar_real_meeting_sources.json`에 고정합니다.
...
- **i18n**: 11개 언어 다국어 지원
- **클라우드 동기화**: 멀티 기기 지원
- **Promise Radar 신뢰도 확장**: 실제 회의/녹음 기반 golden label을 300~500건으로 확대하고 Command Center의 운영 지표를 실사용 로그로 보강
...
**최근 확인**: Promise Radar Command Center backend route/UI model/provider가 추가됐고, Learning Loop v3 상태별 오탐률, Evidence Audit, Google Tasks OAuth guide, focus item 집계가 동작한다. Accuracy fixture는 193건/실제 회의 파생 126건이며 evaluator 193/193, accuracy 1.0을 확인했다. Android `Redmi Note 9 Pro`에는 최신 staging release가 설치됐고 `약속 레이더` 탭이 표시됨을 확인했다. iPhone 17 Pro에서는 Release XCUITest launch smoke와 `permission_microphone_initial`, `permission_denied_recovery`, `unfinished_recording_recovery` Release XCUITest 증거가 PASS했다. Strict release readiness는 production release E2E scenario evidence가 모두 채워져야 최종 통과 가능하다.
