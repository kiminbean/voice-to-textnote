...
- **약속 원장/대시보드**: `GET /api/v1/promise-radar/ledger`, `GET /api/v1/promise-radar/dashboard`, `GET /api/v1/promise-radar/responsibility-scores`, `GET /api/v1/promise-radar/meeting-series`
...
- **정확도 기준선**: 2026-07-01 v11 기준 `promise_radar_accuracy_cases.json` 179건 중 112건은 Creative Commons Town Meeting TV 실제 회의/녹음 파생 label이며 evaluator 결과는 179/179, accuracy 1.0입니다. 출처와 재생성 명령은 `backend/tests/fixtures/promise_radar_real_meeting_sources.json`에 고정합니다.
...
- **i18n**: 11개 언어 다국어 지원
- **클라우드 동기화**: 멀티 기기 지원
- **Promise Radar 운영 자동화**: 담당자 책임 점수, 반복회의 연결, 회의 전 Promise Brief push, Android device gate를 고도화
- **Slack/Google Tasks 연동**: Promise Radar Slack payload/webhook 및 Google Tasks 앱 내 OAuth scope 승인/실제 전송/동기화 지원, Teams/Notion/Jira 확장 후보
