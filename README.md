...
- **자동 판정**: `POST /api/v1/promise-radar/autopilot/{task_id}` (Evidence Lock 적용)
- **학습 루프**: `GET /api/v1/promise-radar/learning-profile`, `POST /api/v1/promise-radar/ledger/{entry_id}/learning-feedback`
- **운영 브리핑**: `GET /api/v1/promise-radar/briefing/pre-meeting`, `GET /api/v1/promise-radar/digest?cadence=daily|weekly`
- **이력/연동/평가**: `GET /api/v1/promise-radar/ledger/{entry_id}/timeline`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task`, `POST /api/v1/promise-radar/accuracy/evaluate`
...
- **i18n**: 11개 언어 다국어 지원
- **클라우드 동기화**: 멀티 기기 지원
- **Slack/Teams 연동**: Promise Radar Slack payload/webhook 1차 연동 완료, Teams/Notion/Jira 확장 후보
...
**최근 확인**: 백엔드 4005 테스트 + Flutter 415 테스트 + Flutter analyze + 기본 Release Readiness 통과. Strict release readiness는 Android signing/Firebase/APNs/App Store Connect secret, Android/iOS 실기기, 실제 E2E evidence가 준비되어야 통과 가능.
