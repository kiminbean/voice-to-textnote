...
- **이력/연동/평가**: `GET /api/v1/promise-radar/ledger/{entry_id}/timeline`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task`, `POST /api/v1/promise-radar/external-task/google-oauth/start`, `POST /api/v1/promise-radar/external-task/google-tasklists`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task/sync`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task/update`, `POST /api/v1/promise-radar/accuracy/evaluate`, `GET /api/v1/promise-radar/accuracy/report`, `GET /api/v1/promise-radar/accuracy/extraction-report`
- **정확도 기준선**: 2026-07-02 v17 기준 `promise_radar_accuracy_cases.json` 629건 중 562건은 Creative Commons YouTube 실제 회의/녹음 파생 label입니다. 영어 Town Meeting TV 회의와 한국어 Welfare TV/DKO 다중 화자 회의가 섞여 있고 evaluator 결과는 629/629, accuracy 1.0입니다. 약속 추출 false negative용 `promise_radar_extraction_cases.json` 50건도 recall 1.0으로 통과합니다. v17은 기존 `.cache/promise-radar-youtube-v15/`, `.cache/promise-radar-audio-v15/`의 YouTube 추출 음성/자막 캐시를 재사용해 한국어 40건, 영어 다중 화자 20건을 추가했습니다. 출처, subtitle cache, 대표 음성 clip 재생성 명령은 `backend/tests/fixtures/promise_radar_real_meeting_sources.json`에 고정합니다. `.cache/promise-radar-youtube-*`, `.cache/promise-radar-audio-*`는 재현용 로컬 캐시이며 git에 포함하지 않습니다.
...
- **i18n**: 11개 언어 다국어 지원
- **클라우드 동기화**: 멀티 기기 지원
- **Promise Radar 신뢰도 확장**: v17에서 실제 회의/녹음 기반 golden label 562건과 extraction recall fixture 50건을 확보했고, production learning telemetry와 Autopilot undo/quarantine을 추가했다. 다음 목표는 production 사용 로그 기반 오탐/누락 샘플링, Google Tasks OAuth callback/token exchange 완성, 실제 팀 회의 label 700건 이상 확장이다.
- **외부 업무도구 연동 확장**: Slack payload/webhook과 Google Tasks one-request token 전송/동기화, OAuth start URL 생성은 구현됨. 다음 후보는 앱 내 OAuth callback 완성도, Google Tasks production 검증, Teams/Notion/Jira 중 1개 실연동
...
**최근 확인**: Promise Radar Command Center backend route/UI model/provider가 v17으로 확장됐고, production learning telemetry, Autopilot undo/quarantine, Live Promise Coach, privacy-safe Evidence Room, Meeting Recipe policy, Google Tasks OAuth start URL, Command Center v17 contract가 동작한다. Accuracy fixture는 629건/실제 회의 파생 562건이며 evaluator 629/629, accuracy 1.0을 확인했다. Extraction recall fixture는 50/50, recall 1.0이다. v17 physical-device E2E generator는 iOS/Android USB 연결 상태에서 `overall_pass=true`로 통과했다. Android `Redmi Note 9 Pro`에는 최신 staging release가 설치됐고 `약속 레이더` 탭이 표시됨을 확인했다. iPhone 17 Pro에서는 Release XCUITest launch smoke와 `permission_microphone_initial`, `permission_denied_recovery`, `unfinished_recording_recovery` Release XCUITest 증거가 PASS했다. Strict release readiness는 production release E2E scenario evidence가 모두 채워져야 최종 통과 가능하다.
