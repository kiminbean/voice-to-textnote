...
✅ **자동화 테스트**: 4005 백엔드 테스트 (단위/통합/E2E) + Flutter 415, 백엔드 100.00% 커버리지
...
- **약속 원장/대시보드**: `GET /api/v1/promise-radar/ledger`, `GET /api/v1/promise-radar/dashboard`, `GET /api/v1/promise-radar/command-center`, `GET /api/v1/promise-radar/responsibility-scores`, `GET /api/v1/promise-radar/meeting-series`
- **자동 판정/검토함**: `POST /api/v1/promise-radar/autopilot/{task_id}`, `GET /api/v1/promise-radar/autopilot/{task_id}/review-queue`, `GET /api/v1/promise-radar/autopilot/quarantine`, `POST /api/v1/promise-radar/ledger/{entry_id}/autopilot-confirm`, `POST /api/v1/promise-radar/ledger/{entry_id}/autopilot-reject`, `POST /api/v1/promise-radar/ledger/{entry_id}/autopilot-undo`
- **학습/정책 루프**: `GET /api/v1/promise-radar/learning-profile`, `GET /api/v1/promise-radar/telemetry/learning`, `GET /api/v1/promise-radar/live-coach`, `GET /api/v1/promise-radar/meeting-recipe`, `POST /api/v1/promise-radar/ledger/{entry_id}/learning-feedback`, `GET|PUT /api/v1/promise-radar/automation-policy`
- **운영 브리핑/푸시**: `GET /api/v1/promise-radar/briefing/pre-meeting`, `POST /api/v1/promise-radar/briefing/pre-meeting/notifications`, `GET /api/v1/promise-radar/digest?cadence=daily|weekly`, `GET|PUT /api/v1/promise-radar/digest-preference`, `POST /api/v1/promise-radar/ledger/notifications/digest`
- **충돌/근거 감사**: `POST /api/v1/promise-radar/ledger/{entry_id}/resolve-conflict`, `GET /api/v1/promise-radar/evidence-room`, `GET /api/v1/promise-radar/ledger/{entry_id}/evidence-pack`, `GET /api/v1/promise-radar/ledger/{entry_id}/evidence-comparison`, `POST /api/v1/promise-radar/ledger/{entry_id}/evidence-room/share-link`, Review Queue Evidence Pack Viewer
- **이력/연동/평가**: `GET /api/v1/promise-radar/ledger/{entry_id}/timeline`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task`, `POST /api/v1/promise-radar/external-task/google-oauth/start`, `POST /api/v1/promise-radar/external-task/google-oauth/callback`, `POST /api/v1/promise-radar/external-task/google-tasklists`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task/sync`, `POST /api/v1/promise-radar/ledger/{entry_id}/external-task/update`, `POST /api/v1/promise-radar/accuracy/evaluate`, `GET /api/v1/promise-radar/accuracy/report`, `GET /api/v1/promise-radar/accuracy/extraction-report`
- **정확도 기준선**: 2026-07-02 v19 기준 `promise_radar_accuracy_cases.json` 1089건 중 1022건은 실제 회의/녹음/공개 transcript 파생 label입니다. hard negative 70건과 public source manifest 24개를 포함하며, 기존 Creative Commons YouTube 영어 Town Meeting TV, 한국어 Welfare TV/DKO 다중 화자 회의, Hugging Face `lytang/MeetingBank-transcript`, `pszemraj/qmsum-cleaned` 기반 sample을 사용합니다. evaluator 결과는 1089/1089, accuracy 1.0이며 `promise_radar_extraction_cases.json` 50건도 recall 1.0으로 통과합니다. 출처, license, subtitle/cache 또는 transcript sample, 재생성 명령은 `backend/tests/fixtures/promise_radar_real_meeting_sources.json`에 고정합니다. `.cache/promise-radar-youtube-*`, `.cache/promise-radar-audio-*`, `.cache/promise-radar-hf-v18/`, `.cache/promise-radar-hf-v19/`는 재현용 로컬 캐시이며 git에 포함하지 않습니다.
...
Firebase, Google Sign-In, iPhone 실기기 로그인, APNs/FCM Push 디버깅 절차는 `docs/google-auth-ios-runbook.md`를 우선 확인합니다. 2026-06-30 APNs/FCM 실제 전송 검증 기록은 `docs/session-summary-2026-06-30-apns-fcm-push.md`에 있습니다.

반복 실수를 막기 위한 핵심 규칙:

- iPhone 홈 화면에서 직접 실행해 테스트할 앱은 Flutter debug 빌드가 아니라 profile 빌드로 설치합니다.
- iPhone/Android에서 로컬 백엔드에 붙을 때 앱의 `localhost`는 Mac이 아닙니다. `API_BASE_URL`은 Mac LAN/Tailscale IP를 사용합니다.
- Android private staging release는 반드시 `--dart-define=ENV=staging`과 `--dart-define=API_BASE_URL=http://100.69.69.119:8000/api/v1`을 함께 지정해 재설치합니다. `약속 레이더` 탭이 보이지 않고 결과 화면 탭이 11개로 보이면 오래된 APK가 설치된 상태입니다.
- Android Promise Radar 메뉴 회귀는 `python3 client/scripts/verify_promise_radar_device_gate.py --serial 76aadc20`로 확인합니다. 이 gate는 설치 metadata, UIAutomator `약속 레이더`/`탭 12개`, APK staging URL guard를 함께 검사합니다.
- iOS 실기기 release evidence는 Debug가 아니라 `xcodebuild test -configuration Release`와 `RunnerUITests` attachment로 수집합니다. iPhone 17 Pro iOS 26.5.1에서 Debug/XCUITest는 Flutter engine `VSyncClient` crash를 재현하지만, 같은 기기에서 Release/XCUITest launch smoke는 PASS했습니다.
...
- 원격 백엔드 PC에서 실제 Push를 테스트하려면 `.env`에 `FIREBASE_CREDENTIALS_PATH`를 유지해야 하며, 현재 Settings 모델이 허용하지 않는 `FIREBASE_PROJECT_ID`는 추가하지 않습니다.
- iOS 앱이 포그라운드일 때는 실제 FCM/APNs 전송이 성공해도 시스템 배너가 표시되지 않을 수 있습니다. 배너 UI 검증은 앱을 백그라운드로 보내거나 화면 잠금 상태에서 수행합니다.
...
iOS Release XCUITest launch evidence 예:
...
| `LLM_PROVIDER` | `zai` | LLM provider. 기본값은 Z.AI Coding Plan |
| `ZAI_BASE_URL` | `https://api.z.ai/api/coding/paas/v4` | Z.AI Coding Plan endpoint |
| `ZAI_API_KEY` | (필수) | Z.AI Coding Plan API 키 |
| `SUMMARY_MODEL` | `glm-5.2` | 요약/감정분석/번역/학습팩/Q&A에 사용할 GLM 모델 |
...
| **iOS** | RC | Release XCUITest launch smoke PASS on iPhone 17 Pro, strict 시나리오별 실기기 evidence 필요 |
| **Android** | RC | `flutter build apk --release` 검증 완료, `Redmi Note 9 Pro` staging release 설치 및 `약속 레이더` 탭 12개 중 4번째 노출 확인, strict 실기기 release evidence 필요 |
...
`--strict`는 예제 파일을 그대로 통과시키는 용도가 아니다. `docs/release-e2e-evidence.example.json`은 참고용 schema fixture이며, strict 입력값은 실제 Android/iOS 기기 ID, 빌드 산출물, Push/딥링크/백그라운드 녹음/HTTP 정책/PDF 공유/Promise Radar Autopilot/Promise Radar due push/Promise Radar calendar export/Promise Radar assignee-quality 시나리오 증거로 채운 `docs/release-e2e-evidence.json`이어야 한다. Android release APK는 `ANDROID_KEYSTORE_BASE64`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`로 복원한 keystore로 서명하고 `REQUIRE_ANDROID_RELEASE_SIGNING=true ./scripts/verify_mobile.sh --native`에서 `apksigner verify --print-certs`까지 통과해야 한다. 이때 certificate DN이 `CN=Android Debug`이면 release APK가 아니므로 gate가 실패해야 한다. iOS는 signed release app에서 `codesign -d --entitlements :-`로 추출한 repo 내부 `IOS_RELEASE_ENTITLEMENTS_PATH`가 `aps-environment=production`, `get-task-allow=false`, App ID/Team ID 일치를 증명해야 한다. release evidence의 `release_gate.ios_entitlements_sha256`은 해당 plist의 SHA-256과 일치해야 하므로, entitlement 증거를 바꾸면 E2E evidence도 다시 생성/갱신해야 한다. 필요한 환경 변수와 scenario key 매핑은 `docs/e2e-device-checklist.md`에 있다.
...
- **Phase 7 완료 항목**: 텍스트 감정 분석 (SPEC-SENTIMENT-001, Z.AI Coding Plan glm-5.2), 음성 톤/운율 분석 (SPEC-TONE-001, opensmile eGeMAPSv02 + librosa), 보안 강화 (SPEC-SEC-002, 매직 바이트 검증 + iOS ATS/Android Network Security)
...
- **i18n**: 11개 언어 다국어 지원
- **클라우드 동기화**: 멀티 기기 지원
- **Promise Radar 신뢰도 확장**: v19에서 실제 회의/녹음/공개 transcript 기반 golden label 1022건, hard negative 70건, extraction recall fixture 50건을 확보했고, Command Center gate를 `target_case_count=1000`으로 올렸다. 다음 목표는 production 사용 로그 기반 오탐/누락 샘플링과 iOS Release XCUITest의 무인 재현성 강화다.
- **외부 업무도구 연동 확장**: Slack payload/webhook과 Google Tasks one-request token 전송/동기화, OAuth start URL, OAuth callback/token exchange가 구현됨. 다음 후보는 Google Tasks production OAuth 실기기 검증, Teams/Notion/Jira 중 1개 실연동
...
**최근 확인**: Promise Radar Command Center backend route/UI model/provider가 v19로 확장됐고, production learning telemetry, hard-negative/public-source accuracy counters, owner identity review telemetry, Digest SLA/push readiness, Memory Graph identity cluster/review, Evidence Room v19 policy, Meeting Recipe policy, Google Tasks OAuth UX/PKCE/token exchange, Command Center v19 contract가 동작한다. Accuracy fixture는 1089건/실제 회의 파생 1022건이며 evaluator 1089/1089, accuracy 1.0을 확인했다. Extraction recall fixture는 50/50, recall 1.0이다. v19 evidence generator는 Command Center `target_case_count=1000`과 실제 label 1000건 이상을 요구한다. Android `Redmi Note 9 Pro`에서는 `promise_radar_android_device_gate_v19.json` 기준 staging release `약속 레이더` 탭, 12개 탭, staging URL guard가 PASS했다. iPhone 17 Pro에서는 iOS 26.5 ProMotion VSync launch crash를 `CADisableMinimumFrameDurationOnPhone=false`로 완화했고, XCUITest stale guest Authorization 404 재발을 막기 위해 UI-test token clearing/result-deeplink bypass를 추가했다. Strict release readiness는 production release E2E scenario evidence가 모두 채워져야 최종 통과 가능하다.
...
✅ SPEC-SENTIMENT-001: 텍스트 감정 분석 — Z.AI Coding Plan glm-5.2 기반 화자별/구간별 감정 분석 + 타임라인 + Flutter 전용 탭
