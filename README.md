...
- **i18n**: 11개 언어 다국어 지원
- **클라우드 동기화**: 멀티 기기 지원
- **Slack/Google Tasks 연동**: Promise Radar Slack payload/webhook 및 Google Tasks 앱 내 OAuth scope 승인/실제 전송 지원, Teams/Notion/Jira 확장 후보
...
**최근 확인**: 백엔드 4005 테스트 + Flutter 415 테스트 + Flutter analyze + 기본 Release Readiness 통과. Android `Redmi Note 9 Pro`에는 최신 staging release가 설치됐고 `약속 레이더` 탭이 표시됨을 확인했다. Strict release readiness는 APNs/App Store Connect secret, production entitlement, Firebase test token, 실제 E2E evidence가 준비되어야 통과 가능.
