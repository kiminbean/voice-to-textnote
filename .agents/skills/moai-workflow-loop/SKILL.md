...
- Hook System: PostToolUse and Stop hooks for seamless Codex integration
...
The PostToolUse hook is triggered after Write and Edit operations. When invoked, Codex provides hook input containing the tool_name (such as Write) and tool_input containing the file_path and content.
...
The Stop hook is triggered after each Codex response. The hook reads the loop state file located at .moai/cache/.moai_loop_state.json. This state contains active status (true or false), current iteration number, max_iterations limit, last_error_count from previous iteration, and completion_reason when finished.
...
For GitHub Actions integration, create a workflow step that runs Codex with the /moai:loop command and max-iterations flag. Set the MOAI_LOOP_ACTIVE environment variable to true to enable loop mode.
...
The LSP hook is located at .Codex/hooks/moai/post_tool__lsp_diagnostic.

The loop hook is located at .Codex/hooks/moai/stop__loop_controller.
...
Status: Active
Integration: Codex Hooks, LSP Protocol, AST-grep
