...
def check_readme_release_status(root: Path, reporter: Reporter) -> None:
    ...
    if (
        "3835 백엔드 테스트" in readme
        and "3835개" in readme
        and ("Flutter 399" in readme or "399개" in readme)
        and "4234개" in readme
    ):
        reporter.ok("README test counts match current release validation evidence")
    else:
        reporter.fail(
            "README test counts must match current 3835 backend / 399 Flutter / 4234 total evidence"
        )
...
def check_docs(root: Path, reporter: Reporter) -> None:
    ...
    if "3835 passed" in procedure_doc and "Flutter: 399 passed" in procedure_doc:
        reporter.ok("Release procedure backend test count matches latest full pytest evidence")
    else:
        reporter.fail(
            "Release procedure test counts must match latest 3835 backend / 399 Flutter evidence"
        )
