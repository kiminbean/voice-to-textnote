...
def check_readme_release_status(root: Path, reporter: Reporter) -> None:
    ...
    if (
        ...
        and "3835개" in readme
        and ("Flutter 396" in readme or "396개" in readme)
        and "4231개" in readme
    ):
        reporter.ok("README test counts match current release validation evidence")
    else:
        reporter.fail(
            "README test counts must match current 3835 backend / 396 Flutter / 4231 total evidence"
        )
...
def check_docs(root: Path, reporter: Reporter) -> None:
    ...
    if "3835 passed" in procedure_doc and "Flutter: 396 passed" in procedure_doc:
        reporter.ok("Release procedure backend test count matches latest full pytest evidence")
    else:
        reporter.fail(
            "Release procedure test counts must match latest 3835 backend / 396 Flutter evidence"
        )
