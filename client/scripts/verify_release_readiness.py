...
def check_readme_release_status(root: Path, reporter: Reporter) -> None:
    ...
    if (
        ...
        reporter.fail(
            "README test counts must match current 3835 backend / 391 Flutter / 4226 total evidence"
        )
...
def check_docs(root: Path, reporter: Reporter) -> None:
    ...
    if "3835 passed" in procedure_doc and "Flutter: 391 passed" in procedure_doc:
        reporter.ok("Release procedure backend test count matches latest full pytest evidence")
    else:
        reporter.fail(
            "Release procedure test counts must match latest 3835 backend / 391 Flutter evidence"
        )
