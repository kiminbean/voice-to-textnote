...
def check_readme_release_status(root: Path, reporter: Reporter) -> None:
    ...
    if (
        "3840 백엔드 테스트" in readme
        and "3840개" in readme
        and ("Flutter 401" in readme or "401개" in readme)
        and "4241개" in readme
    ):
        reporter.ok("README test counts match current release validation evidence")
    else:
        reporter.fail(
            "README test counts must match current 3840 backend / 401 Flutter / 4241 total evidence"
        )
...
def check_docs(root: Path, reporter: Reporter) -> None:
    ...
    if "3840 passed" in procedure_doc and "Flutter: 401 passed" in procedure_doc:
        reporter.ok("Release procedure backend test count matches latest full pytest evidence")
    else:
        reporter.fail(
            "Release procedure test counts must match latest 3840 backend / 401 Flutter evidence"
        )
