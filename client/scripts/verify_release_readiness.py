...
def check_readme_release_status(root: Path, reporter: Reporter) -> None:
    ...
    if (
        "3820 백엔드 테스트" in readme
        and "3820개" in readme
        and ("Flutter 391" in readme or "391개" in readme)
        and "4211개" in readme
    ):
        reporter.ok("README test counts match current release validation evidence")
    else:
        reporter.fail(
            "README test counts must match current 3820 backend / 391 Flutter / 4211 total evidence"
        )
...
def check_docs(root: Path, reporter: Reporter) -> None:
    ...
    if "3820 passed" in procedure_doc and "Flutter: 391 passed" in procedure_doc:
        reporter.ok("Release procedure backend test count matches latest full pytest evidence")
    else:
        reporter.fail(
            "Release procedure test counts must match latest 3820 backend / 391 Flutter evidence"
        )
