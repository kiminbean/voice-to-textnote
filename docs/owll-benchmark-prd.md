...
**Status**: Study Pack core implemented; Cross-Meeting Q&A evidence search and synthesis exposed in search; sales follow-up briefs are searchable and listable; 2026-06-30 benchmark refreshed; Promise Radar v19 Command Center, 1000+ real-meeting/transcript accuracy baseline, hard-negative gate, extraction recall gate, production learning telemetry, Autopilot undo/quarantine, Live Coach, privacy/evidence gate, Google Tasks OAuth UX readiness, and evidence loop implemented
**Created**: 2026-06-21  
...
- The tab shows:
  ...
  - v17 controls for timeline, learning feedback (`오판`), production learning telemetry, Autopilot undo/quarantine, Live Promise Coach, Slack export, Google Tasks OAuth start/tasklist send/sync/update, Autopilot Review Queue filters/reject persistence, conflict signal comparison, Evidence Pack Viewer, Evidence Comparison, privacy-safe Evidence Room, Meeting Recipe policy, team automation policy, Digest preference, 560+ real-meeting accuracy report, 50-case extraction recall report, Promise Memory Graph, Autopilot Shadow Mode, Evidence Permission gate, Team Scorecard, and the Command Center focus/action/evidence/learning panels
...
- v17 response additions:
  - `PromiseLearningTelemetryReport`: privacy-safe production telemetry by status, owner, locale, and payload shape.
  - `PromiseAutopilotUndoResponse` and `PromiseAutopilotQuarantineSummary`: restores the pre-Autopilot ledger snapshot and keeps repeated bad status suggestions in review.
  - `PromiseLiveCoachSummary`: meeting-time prompts that ask for owner, due date, status, and completion criteria.
  - `PromiseEvidenceRoomSummary` and `PromiseEvidenceRoomLinkResponse`: redacted Evidence Pack sharing with short TTL and no raw token logging.
  - `PromiseMeetingRecipePolicy`: meeting-type policy for owner/due/evidence/autopilot behavior.
  - `PromiseGoogleTasksOAuthStartResponse`: Google Tasks authorization URL, state, redirect URI, scope, setup readiness, and token-handling guidance.
...
- Given the Command Center is opened, the app loads dashboard, review queue, learning insight, production learning telemetry, digest, pre-meeting brief, Live Promise Coach, external-task reconcile, accuracy report, extraction recall report, Evidence Audit, Promise Memory Graph, Autopilot Shadow Mode, Evidence Permission gate, Evidence Room, Team Scorecard, Autopilot Quarantine, Meeting Recipe, Google Tasks OAuth guide/readiness, operator actions, and prioritized focus items from one endpoint.
