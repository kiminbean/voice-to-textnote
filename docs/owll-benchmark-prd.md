...
| Contact manager for sales notes | Sales follow-up summary mode, Sales Contact Brief API, Result-screen tab, searchable/cross-meeting discoverable sales brief artifacts, backend customer list API, Flutter lifecycle filters, business-card OCR intake, copyable contact-field confirmation, and editable CRM status/notes implemented | Native camera capture and richer CRM sync remain | P3 |
