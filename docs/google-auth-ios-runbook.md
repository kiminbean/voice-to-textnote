...
| Android Google 계정 선택 후 `not registered to use OAuth2.0` 또는 `ApiException: 10` | 실제 설치 APK의 package/SHA-1 조합이 Google Cloud Console Android OAuth client에 등록되어 있지 않음 | 설치 APK SHA-1을 확인하고 `com.voicetextnote.app` Android OAuth client에 등록한 뒤 `google-services.json`을 다시 내려받아 release APK를 재설치한다. 2026-07-02 release upload key SHA-1은 `45:6F:94:A6:F0:78:99:7A:8D:E9:DB:9C:C2:F1:03:B7:FF:BE:B7:4C`. |
