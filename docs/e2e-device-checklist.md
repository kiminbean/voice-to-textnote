...
- `promise_radar_command_center`: `약속 레이더 Command Center` 화면이 dashboard, focus item, operator actions, Autopilot Review Queue, Learning Loop v3, Production Learning Telemetry, Live Promise Coach, Autopilot Undo/Quarantine, Evidence Audit, Daily Digest, Google Tasks OAuth guide/readiness, accuracy report, extraction recall report, Promise Memory Graph, Autopilot Shadow Mode, Evidence Permission, Privacy-Safe Evidence Room, Meeting Recipe, Team Scorecard를 한 화면에서 표시하는지 확인한다.
...
- `promise_radar_google_tasks_oauth_start`: `POST /api/v1/promise-radar/external-task/google-oauth/start`가 Google Tasks scope authorization URL, state, redirect URI, missing setup, token handling 안내를 반환하는지 확인한다.
- `promise_radar_learning_telemetry`: `GET /api/v1/promise-radar/telemetry/learning`와 Command Center가 상태/담당자/언어/payload shape별 sample, false-positive, correction telemetry를 표시하는지 확인한다.
- `promise_radar_autopilot_undo_quarantine`: `POST /api/v1/promise-radar/ledger/{entry_id}/autopilot-undo` 후 원장 상태가 이전 snapshot으로 복구되고 `GET /api/v1/promise-radar/autopilot/quarantine`에 같은 상태 후보가 격리되는지 확인한다.
