...
- Preserved release-gate honesty: iPhone evidence is PASS, Android physical-device PASS is not claimed because no ADB device was connected.
...
- Added `backend/scripts/generate_promise_radar_e2e_evidence.py`.
  ...
  - Captures iPhone/Android device state and redacts hardware serial, ECID, UDID, and tunnel IP values.
...
- `backend/scripts/generate_promise_radar_e2e_evidence.py` -> `overall_pass: true`.

## Remaining Release Notes

- `client/scripts/verify_release_readiness.py --strict` still fails without production release inputs: Android release signing, Firebase service account, APNs/App Store Connect keys, physical Android serial, iOS UDID env, production entitlements evidence, Firebase test token, and full release E2E evidence.
- The app intentionally uses `100.69.69.119` for local/backend-server testing; strict App Store/Play release HTTP policy still expects no cleartext IP exception.
