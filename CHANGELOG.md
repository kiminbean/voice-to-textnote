...
- **SPEC-SENTIMENT-001: 감정 분석 on-demand 패턴修復** — result_provider.dart가 POST /sentiment → poll → GET /sentiment/{task_id} 패턴으로 변경 (기존: 잘못된 GET /sentiment/meeting/{id} 호출로 항상 실패)
