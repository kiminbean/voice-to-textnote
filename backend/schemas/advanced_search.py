...
class SearchFilter(BaseModel):
    ...
    start_date: datetime | None = Field(default=None, description="검색 시작 일시 (UTC)")
    end_date: datetime | None = Field(default=None, description="검색 종료 일시 (UTC)")
    speaker_ids: list[str] | None = Field(default=None, description="화자 ID 목록 (필터링)")
    content_types: list[str] | None = Field(
        default=None, description="콘텐츠 유형 필터 (minutes, summary)"
    )
    min_word_count: int | None = Field(default=None, description="최소 단어 수")
    max_word_count: int | None = Field(default=None, description="최대 단어 수")
    tags: list[str] | None = Field(default=None, description="태그 필터")
...
class AdvancedSearchRequest(BaseModel):
    ...
    sort_by: str = Field(
        default="relevance", description="정렬 기준 (relevance, date, speaker, word_count)"
    )
