...
class SummaryMode(StrEnum):
    ...
    LECTURE_NOTES = "lecture_notes"  # 강의 노트
    SALES_FOLLOW_UP = "sales_follow_up"  # 영업 후속 조치
    SERMON_NOTES = "sermon_notes"  # 설교 노트
...
class SummaryRequest(BaseModel):
    ...
    summary_mode: SummaryMode = Field(default=SummaryMode.EXECUTIVE, description="요약 모드")
    length: SummaryLength = Field(default=SummaryLength.MEDIUM, description="요약 길이")
    focus_areas: list[FocusArea] = Field(default=[FocusArea.ALL], description="집중 영역 목록")
    include_sentiment: bool = Field(default=True, description="감정 분석 포함 여부")
    generate_multiple_versions: bool = Field(default=False, description="여러 버전의 요약 생성")
    target_audience: str | None = Field(
        default=None, description="요청 대상 (예: 경영진, 개발자, 팀원)"
    )
...
class SentimentAnalysis(BaseModel):
    ...
    sentiment_score: float = Field(
        ge=-1.0, le=1.0, description="감정 점수 (-1: 부정, 0: 중립, 1: 긍정)"
    )
...
class SummaryGenerationResult(BaseModel):
    ...
    alternative_versions: list[VersionedSummary] = Field(
        default_factory=list, description="대체 버전"
    )
