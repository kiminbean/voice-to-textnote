        ...
        with patch(
            ...
            with pytest.raises(VoiceNoteError):
                await get_live_quality_score(
                    task_id="test-task", persist=True,
                    db=AsyncMock(), svc=mock_svc,
                )
    ...
    async def test_submit_feedback_voice_note_error_reraise(self):
        ...
        with pytest.raises(VoiceNoteError):
            await submit_quality_feedback(
                task_id="test-task", payload=MagicMock(),
                db=mock_db, svc=mock_svc,
            )
    ...
    async def test_list_feedback_voice_note_error_reraise(self):
        ...
        with pytest.raises(VoiceNoteError):
            await list_quality_feedback(
                task_id="test-task", recent_limit=10,
                db=AsyncMock(), svc=mock_svc,
            )
    ...
    async def test_quality_trends_voice_note_error_reraise(self):
        ...
        with pytest.raises(VoiceNoteError):
            await get_quality_trends(
                task_id="test-task", limit=50, warning_drop_threshold=None,
                db=AsyncMock(), svc=mock_svc,
            )
...
class TestEnhancedPreprocessFull:
    ...
    async def test_single_preprocess_success(self):
        ...
        mock_result = BatchPreprocessResult(
            task_id="single-ok", total_files=1, processed_files=1,
            failed_files=0, processing_time_seconds=0.5,
        ...
        with patch(
            ...
            audio_preprocess_enabled=True, audio_preprocess_max_file_mb=100,
        ), patch(
            ...
            resp = await enhanced_preprocess_endpoint(
                file=mock_file,
                convert_to_16k_mono=True, normalize=True, target_dbfs=-20.0,
                high_pass_hz=None, low_pass_hz=None, trim_silence=False,
                silence_threshold_db=-40.0, silence_min_len_ms=700,
                ai_noise_removal=True, noise_threshold=0.1, denoise_strength=0.8,
            )
    ...
    async def test_single_preprocess_failed_400(self):
        ...
        mock_result = BatchPreprocessResult(
            task_id="single-fail", total_files=1, processed_files=0,
            failed_files=1, processing_time_seconds=0.1,
            results=[], errors=[{"file": "bad.wav", "error": "실패"}], summary={},
        )
    ...
    async def test_batch_with_report(self):
        ...
        mock_result = BatchPreprocessResult(
            task_id="batch-ok", total_files=1, processed_files=1,
            failed_files=0, processing_time_seconds=1.0,
            results=[], errors=[], summary=_FULL_SUMMARY,
        )
        ...
        with patch(
            ...
            resp = await batch_preprocess_endpoint(
                ...
                silence_threshold_db=-40.0, silence_min_len_ms=700,
                ai_noise_removal=True, noise_threshold=0.1, denoise_strength=0.8,
                output_format="zip", return_report=True,
            )
    ...
    async def test_batch_without_report(self):
        ...
        mock_result = BatchPreprocessResult(
            task_id="batch-nr", total_files=1, processed_files=1,
            failed_files=0, processing_time_seconds=1.0,
            results=[], errors=[], summary=_FULL_SUMMARY,
        )
        ...
        with patch(
            ...
            resp = await batch_preprocess_endpoint(
                ...
                silence_threshold_db=-40.0, silence_min_len_ms=700,
                ai_noise_removal=True, noise_threshold=0.1, denoise_strength=0.8,
                output_format="zip", return_report=False,
            )
