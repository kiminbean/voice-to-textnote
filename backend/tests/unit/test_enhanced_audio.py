...
class TestEnhancedAudioProcessor:
    ...
    async def test_preprocess_single_file(self, sample_audio_file):
        ...
        try:
            ...
            assert isinstance(audio_info, AudioFileInfo)
            assert audio_info.original_path == Path(sample_audio_file)
            assert audio_info.processed_path == output_path
    ...
    async def test_ai_noise_removal_simulation(self, sample_audio_file):
        ...
        assert isinstance(numpy_array, type([]))  # 리스트로 변환됨
...
class TestEnhancedAudioAPI:
    ...
    async def test_get_supported_formats(self, client):
        """지원 오디오 포맷 조회 테스트"""
        response = client.get("/api/v1/audio/enhanced/formats")
    ...
    async def test_get_model_status(self, client):
        """AI 모델 상태 조회 테스트"""
        response = client.get("/api/v1/audio/enhanced/status")
        assert response.status_code == 200
    ...
    async def test_enhanced_preprocess_validation(self):
        ...
        # 무효한 값 테스트
        # target_dbfs 범위 외 값
        invalid_options = EnhancedPreprocessOptions(target_dbfs=-100.0)
        assert invalid_options.target_dbfs == -60.0  # 자동 조정
        
        # high_pass_hz 범위 외 값
        invalid_options = EnhancedPreprocessOptions(high_pass_hz=1000)
        assert invalid_options.high_pass_hz is None  # 자동 조정
        
        # noise_threshold 범위 외 값
        invalid_options = EnhancedPreprocessOptions(noise_threshold=1.5)
        assert invalid_options.noise_threshold == 1.0  # 자동 조정
    ...
    async def test_audio_info_model_validation(self):
        ...
        assert audio_info.original_path == Path("/path/to/original.wav")
        assert audio_info.processed_path == Path("/path/to/processed.wav")
