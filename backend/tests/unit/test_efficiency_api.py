...
class TestEfficiencyAPI:
    ...
    def test_efficiency_api_integration(self):
        ...
        # 실제 API 호출 대신 엔드포인트 구조 검증
        # 실제 테스트 데이터베이스 연결 없이
        
        # API 라우터가 올바르게 설정되었는지 확인
        api_routes = [route.path for route in app.routes if hasattr(route, 'path')]
        
        # analytics 엔드포인트가 있는지 확인
        analytics_endpoints = [route for route in api_routes if '/analytics/' in route]
        assert len(analytics_endpoints) > 0, "No analytics endpoints found"
        
        # efficiency 엔드포인트 특성 검증
