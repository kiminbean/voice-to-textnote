...
class TestPreprocessEndpoint:
    ...
    def test_invalid_format_returns_400(self, app_client):
        """지원하지 않는 포맷 → 400."""
        with (
            patch("backend.app.api.v1.audio.audio_preprocess.settings") as mock_s,
            patch(
    ...
    def test_invalid_options_returns_422(self, app_client):
        ...
        with (
            ...
            with patch(
                "backend.app.api.v1.audio.audio_preprocess.PreprocessOptions"
            ) as mock_opts_cls:
                mock_opts = MagicMock()
                ...
                with patch(
                    "backend.app.api.v1.audio.audio_preprocess.PreprocessOptionsPayload"
                ) as mock_payload:
    ...
    def test_file_too_large_returns_413(self, app_client):
        ...
        with (
            ...
            with patch(
                "backend.app.api.v1.audio.audio_preprocess._resolve_options"
            ) as mock_resolve:
    ...
    def test_preprocess_value_error_returns_400(self, app_client):
        ...
        with (
            ...
            patch(
                "backend.app.api.v1.audio.audio_preprocess.preprocess_audio",
                side_effect=ValueError("잘못된 오디오"),
            ),
            patch("backend.app.api.v1.audio.audio_preprocess._preprocess_semaphore"),
        ):
            mock_s.audio_preprocess_enabled = True
