...
def sample_segments():
    return [
        ...
        },
        {
            ...
            "end": 12.0,
            "speaker_name": "이과장",
            "text": "네, 이번 프로젝트 진행 상황 공유드리겠습니다.",
        },
        {
            "start": 12.0,
            "end": 20.0,
            "speaker_name": "박부장",
            "text": "일정이 계속 지연되고 있는데, 이게 어떻게 된 건가요?",
        },
        {
            "start": 20.0,
            "end": 28.0,
            "speaker_name": "김대리",
            "text": "죄송합니다. 리소스 부족 문제가 있었습니다.",
        },
        {
            "start": 28.0,
            "end": 35.0,
            "speaker_name": "이과장",
            "text": "하지만 이번 주까지는 완료할 수 있습니다! 긍정적으로 봐주세요.",
        },
    ]
...
class TestSpeakerStats:
    def test_correct_ratios(self, analyzer):
        segments = [
            SentimentSegment(
                start=0, end=5, speaker="A", sentiment="positive", emotion="joy", confidence=0.9
            ),
            SentimentSegment(
                start=5,
                end=10,
    ...
    def test_multiple_speakers(self, analyzer):
        segments = [
            ...
            SentimentSegment(
                start=5, end=10, speaker="B", sentiment="negative", emotion="anger", confidence=0.8
            ),
        ]
        speakers = analyzer._compute_speaker_stats(segments)
