...
class TestActionItemSchemaClassDefs:
    ...
    def test_action_item_response(self):
        ...
        r = ActionItemResponse(
            ...
            assignee_id=uid, assignee_name="U",
            created_by=uid, created_by_name="U",
            created_at=now, updated_at=now,
            due_date=None, completed_at=None,
            completed_by=None, completed_by_name=None,
            completion_notes=None, meeting_id=None, meeting_title=None,
            tags=["test"], estimated_hours=None, actual_hours=None,
            category=None, is_overdue=False,
            time_remaining_hours=None, progress_percentage=0.0,
        )
    ...
    def test_action_item_comment(self):
        now = datetime.utcnow()
        c = ActionItemComment(
            id=uuid4(), action_item_id=uuid4(), author_id=uuid4(),
            author_name="U", content="c", created_at=now,
            updated_at=now, is_internal=False,
        )
        assert c.content == "c"
    ...
    def test_action_item_history(self):
        h = ActionItemHistory(
            id=uuid4(), action_item_id=uuid4(), field_name="status",
            old_value="pending", new_value="done",
            changed_by=uuid4(), changed_by_name="U",
            changed_at=datetime.utcnow(), change_type="update",
        )
        assert h.field_name == "status"

    def test_action_item_reminder(self):
        r = ActionItemReminder(
            id=uuid4(), action_item_id=uuid4(), reminder_type="before_due",
            reminder_time=datetime.utcnow(), is_active=True,
            last_sent_at=None, created_at=datetime.utcnow(),
        )
