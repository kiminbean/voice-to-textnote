...
class TestTriggerCleanup:
    ...
    def test_cleanup_returns_200(self, client):
        ...
        with (
            patch("backend.app.api.v1.admin.cleanup_expired_results", return_value=5),
            patch("backend.app.api.v1.admin.cleanup_temp_files", return_value=(3, 1024 * 1024)),
            patch("backend.app.api.v1.admin.get_sync_session") as mock_session_cm,
        ):
    ...
    def test_cleanup_returns_result_counts(self, client):
        ...
        with (
            patch("backend.app.api.v1.admin.cleanup_expired_results", return_value=10),
            patch("backend.app.api.v1.admin.cleanup_temp_files", return_value=(7, 2 * 1024 * 1024)),
            patch("backend.app.api.v1.admin.get_sync_session") as mock_session_cm,
        ):
            mock_session = MagicMock()
            mock_session_cm.return_value.__enter__ = MagicMock(return_value=mock_session)
    ...
    def test_cleanup_returns_freed_bytes(self, client):
        ...
        with (
            patch("backend.app.api.v1.admin.cleanup_expired_results", return_value=0),
            patch("backend.app.api.v1.admin.cleanup_temp_files", return_value=(3, freed_bytes)),
            patch("backend.app.api.v1.admin.get_sync_session") as mock_session_cm,
        ):
            mock_session = MagicMock()
            mock_session_cm.return_value.__enter__ = MagicMock(return_value=mock_session)
    ...
    def test_cleanup_zero_results_when_nothing_to_delete(self, client):
        ...
        with (
            patch("backend.app.api.v1.admin.cleanup_expired_results", return_value=0),
            patch("backend.app.api.v1.admin.cleanup_temp_files", return_value=(0, 0)),
            patch("backend.app.api.v1.admin.get_sync_session") as mock_session_cm,
        ):
            mock_session = MagicMock()
            mock_session_cm.return_value.__enter__ = MagicMock(return_value=mock_session)
