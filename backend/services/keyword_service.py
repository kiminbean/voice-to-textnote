...
class KeywordService:
    ...
    async def suggest_keywords(
        ...
        for keyword, freq in frequency_data.items():
            ...
            synonyms = self._find_synonyms(keyword) if include_synonyms else []
    ...
    def _score_keywords(
        ...
        """빈도 기반 점수 계산 후 KeywordItem 리스트 반환"""
        counter = Counter(keywords)
        total = sum(counter.values()) or 1
        items: list[KeywordItem] = []
        for kw, freq in counter.most_common():
            score = round(min(1.0, (freq / total) * 2 + math.log(freq + 1) / 5), 4)
            items.append(KeywordItem(keyword=kw, score=score, frequency=freq))
        return items
    ...
    def extract_from_text(
        ...
        *,
        ...
        raw = self._extract_keywords(text)
        scored = self._score_keywords(text, raw)
        scored = [k for k in scored if k.score >= min_score][:max_keywords]
    ...
    async def extract_for_task(
        ...
        *,
        ...
        max_kw = max_keywords or 20
        min_sc = min_score if min_score is not None else 0.3

        cache_key = f"keywords:{task_id}"
        cached = await redis_client.get(cache_key)
        if cached:
            import json
            data = json.loads(cached)
            resp = KeywordResponse.model_validate(data)
            return resp.model_copy(
                update={"keywords": [k for k in resp.keywords if k.score >= min_sc][:max_kw]}
            )

        from sqlalchemy import select as sa_select

        result = await db.execute(
            sa_select(TaskResult).where(TaskResult.task_id == task_id)
        )
        task_row = result.scalars().first()

        if not task_row:
            return KeywordResponse(keywords=[], total_count=0, source="task", language="ko")

        content = self._extract_text_from_result(task_row.result_data or {})
        if not content:
            return KeywordResponse(keywords=[], total_count=0, source="task", language="ko")

        resp = self.extract_from_text(
            content, language="ko", max_keywords=max_kw, min_score=min_sc, source="task"
        )

        import json
        await redis_client.setex(cache_key, 86400, resp.model_dump_json())  # 24h TTL
    ...
    async def recommend_for_task(
        ...
        *,
        ...
        current = await self.extract_for_task(
        ...
        try:
            result = await db.execute(
                __import__("sqlalchemy").select(TaskResult)
        ...
        for rt in recent_tasks:
            content = self._extract_text_from_result(rt.result_data or {})
            history_keywords.extend(self._extract_keywords(content))

        all_keywords = [k.keyword for k in current.keywords] + history_keywords
        scored = self._score_keywords(" ".join(all_keywords), list(set(all_keywords)))
        scored = [k for k in scored if k.score >= min_score][:max_keywords]
