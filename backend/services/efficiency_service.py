...
class EfficiencyService:
    ...
    async def analyze_meeting_efficiency(
        ...
        # 2. 기본 지표 계산
        self._calculate_basic_metrics(segments, analysis_depth)
        ...
        action_item_metric = await self._analyze_action_items(segments, db, task_id)
        time_distribution = self._analyze_time_distribution(segments, analysis_depth)
        keyword_metric = self._analyze_keywords(segments)
        sentiment_trend = (
            self._analyze_sentiment(segments) if analysis_depth == "detailed" else None
        )
        ...
        if include_recommendations:
            recommendations = self._generate_recommendations(
                efficiency_metrics, participation_metrics, time_distribution, analysis_depth
            )
    ...
    async def _get_minutes_data(
        self, redis_client: aioredis.Redis, db: AsyncSession, task_id: str
    ) -> dict | None:
        """회의록 데이터 조회 (Redis 우선)"""
        # Redis 조회
    ...
    def _analyze_participation(
        self, segments: list[dict], min_speakers: int
    ) -> list[ParticipationMetric]:
        """화자 참여도 분석"""
        speaker_stats = {}
        ...
        for seg in segments:
            ...
            if speaker not in speaker_stats:
                speaker_stats[speaker] = {"total_time": 0, "segment_count": 0, "total_words": 0}
        ...
        for speaker, stats in speaker_stats.items():
            ...
            speaking_percentage = (
                (stats["total_time"] / total_meeting_time * 100) if total_meeting_time > 0 else 0
            )
            avg_segment_length = (
                stats["total_time"] / stats["segment_count"] if stats["segment_count"] > 0 else 0
            )

            # 참여도 균형 점수 (참여 시간이 평균에 가까울수록 높음)
            avg_participation = total_meeting_time / len(speaker_stats)
            participation_balance_score = (
                1 - abs(stats["total_time"] - avg_participation) / avg_participation
                if avg_participation > 0
                else 0
            )
            participation_balance_score = max(0, participation_balance_score)

            participation_metrics.append(
                ParticipationMetric(
                    speaker_id=speaker,
    ...
    def _analyze_decisions(self, segments: list[dict], analysis_depth: str) -> DecisionMetric:
        ...
        total_decisions = len(
            set(
                ...
                for seg in [segments[i] for i in decision_segments if i < len(segments)]
            )
        )
        ...
        total_duration = max(seg.get("end_time", 0) for seg in segments)
        decisions_per_hour = (
            (total_decisions / (total_duration / 3600)) if total_duration > 0 else 0
        )
        ...
        # 결정 구간 평균 길이
        decision_times = [
            segments[i].get("end_time", 0) - segments[i].get("start_time", 0)
            for i in decision_segments
            if i < len(segments)
        ]
    ...
    async def _analyze_action_items(
        self, segments: list[dict], db: AsyncSession, task_id: str
    ) -> ActionItemMetric:
        """액션 아이템 분석"""
        # DB에서 액션 아이템 조회
        stmt = select(ActionItem).where(ActionItem.meeting_id == task_id)
        result = await db.execute(stmt)
        ...
        # 시간당 액션 아이템 수
        total_duration = max(seg.get("end_time", 0) for seg in segments)
        action_items_per_hour = (
            (total_action_items / (total_duration / 3600)) if total_duration > 0 else 0
        )

        # 액션 아이템 명확도 점수
        clear_action_items = sum(
            1 for item in action_items if item.description and len(item.description.strip()) > 10
        )
    ...
    def _analyze_time_distribution(
        self, segments: list[dict], analysis_depth: str
    ) -> TimeDistributionMetric:
        """시간 분배 분석"""
        if not segments:
        ...
        actual_speaking_time = sum(
            seg.get("end_time", 0) - seg.get("start_time", 0) for seg in segments
        )
        silence_percentage = (
            ((total_meeting_duration - actual_speaking_time) / total_meeting_duration * 100)
        ...
        if len(segment_durations) > 1:
            ...
            agenda_adherence_score = max(
                0,
                1 - (duration_variance / statistics.mean(segment_durations))
                if statistics.mean(segment_durations) > 0
                else 0.5,
            )
        else:
            agenda_adherence_score = 0.5
        ...
        time_efficiency_score = (
            actual_speaking_time / total_meeting_duration if total_meeting_duration > 0 else 0
        )
            ...
            for category, keywords in self.keyword_patterns.items():
                for keyword in keywords:
                    if keyword in text:
                        ...
                        all_keywords.append(
                            {"keyword": keyword, "category": category, "speaker": speaker}
                        )
    ...
    def _analyze_sentiment(self, segments: list[dict]) -> SentimentTrend | None:
        """감정 분석 (간단한 구현)"""
        ...
        return SentimentTrend(
            ...
            sentiment_volatility=round(min(1.0, sentiment_volatility), 2),
        )
    ...
    def _calculate_efficiency_score(
        ...
        if participation_metrics:
            avg_participation_balance = statistics.mean(
                [pm.participation_balance_score for pm in participation_metrics]
            )
        else:
            avg_participation_balance = 0

        # 결정 효과성 점수
        decision_effectiveness = decision_metric.decision_clarity_score * min(
            1.0, decision_metric.decisions_per_hour / 5
        )

        # 액션 아이템 품질 점수
        action_item_quality = action_item_metric.action_item_clarity_score * min(
            1.0, action_item_metric.action_items_per_hour / 3
        )
    ...
    def _generate_recommendations(
        ...
        if efficiency_metrics.participation_balance < 0.7:
            unbalanced_speakers = [
                pm for pm in participation_metrics if pm.participation_balance_score < 0.5
            ]
            if unbalanced_speakers:
                recommendations["participation"].append(
                    Recommendation(
                        ...
                        specific_actions=[
                            f"'{unbalanced_speakers[0].speaker_name}' 참여 시간 조정 (현재 {unbalanced_speakers[0].speaking_percentage:.1f}%)",
                            "참여 균형을 위한 발화 기회 설정",
                            "패시브 참여자 적극 유도 방법 마련",
                        ],
            ...
            recommendations["time"].append(
                Recommendation(
                    ...
                    expected_improvement=(
                        f"시간 효율성 {efficiency_metrics.time_efficiency:.1f}% → 0.9+ 향상"
                    ),
                )
            )
        ...
        if efficiency_metrics.decision_effectiveness < 0.7:
            recommendations["decision"].append(
                Recommendation(
                    category="decision",
                    priority="medium",
                    title="결정 프로세스 개선",
        ...
        if efficiency_metrics.action_item_quality < 0.7:
            recommendations["action_item"].append(
                Recommendation(
        ...
        if efficiency_metrics.participation_balance < 0.6:
            quick_wins.append("라운드식 발화 기회 제공")
