...
class AudioEnhancementService:
    ...
    def _read_audio_file(self, file_path: Path) -> Tuple[np.ndarray, int]:
        ...
        try:
            ...
            logger.error(f"오디오 파일 읽기 실패: {file_path}", error=str(e))
            raise
    ...
    def _apply_noise_reduction(self, audio: np.ndarray, level: NoiseReductionLevel) -> np.ndarray:
        ...
        # 간단한 노이즈 게이트 적용
        noise_gate = np.abs(audio) > self.noise_gate_threshold
    ...
    def _calculate_quality_scores(self, original_audio: np.ndarray, enhanced_audio: np.ndarray, 
        ...
        enhanced_fft = np.fft.fft(enhanced_audio)
        high_freq_mask = np.fft.fftfreq(len(original_audio)) > 0.5  # 고주파 마스크
        clarity_score = min(np.mean(np.abs(enhanced_fft[high_freq_mask]) / 
        ...
        volume_level = enhanced_rms / (original_rms + 1e-8)
        ...
        overall_score = (clarity_score * 0.4 + (1 - noise_level) * 0.3 + 
                        min(volume_level, 1.0) * 0.2 + speech_ratio * 0.1)
    ...
    async def enhance_audio(self, file_path: Path, request: AudioEnhancementRequest) -> EnhancementResult:
        ...
        if request.target_sample_rate and sr != request.target_sample_rate:
            processed_audio = librosa.resample(
                processed_audio, orig_sr=sr, target_sr=request.target_sample_rate
            )
