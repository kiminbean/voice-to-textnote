"""
AI 회의 요약 생성기 - ZAI-compatible LLM API 기반
REQ-SUM-001: MinutesSegment[], SpeakerStats[]로 LLM 프롬프트 빌드
REQ-SUM-002: LLM 응답을 구조화된 결과로 파싱
...
class SummaryGenerator:
    """
    ZAI-compatible LLM API를 사용하여 회의 요약을 생성하는 클래스
    """
...
{format_instruction}{mandatory_instruction}"""
    ...
    def generate_summary(
        ...
        """
        ZAI-compatible LLM API를 호출하여 회의 요약 생성

        Args:
            segments: MinutesSegment dict 목록
            speaker_stats: SpeakerStats dict 목록
            api_key: ZAI-compatible API 키
            model: ZAI-compatible 모델명 (glm-5.2 등)
